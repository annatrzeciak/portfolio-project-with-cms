...
if(($_SESSION['user_id']>0)){
  ...
  <table class="table table-striped table-hover"><thead><tr><th>lp.</th><th>Tytuł</th><th>Opis</th><th>Tagi</th><th>Link do projektu</th><th>Link do kodu</th><th>Zdjęcie</th><th class="actionCell">Akcje</th></thead><tbody>';
    ...
    echo '<tr><td>'.$key.'</td><td>'.$value[1].'</td><td>'.$value[2].'</td><td>'.$value[6].'</td><td>'.$value[4].'</td><td>'.$value[5].'</td><td><img width="150" src="';
    echo ($value[3]==''|| $value[3]=='/images/')? '/images/default-image.png' : $value[3];

    echo '"></td><td class="actionCell"><a href="editProject.php?id='.$value[0].' "><i class="fas fa-edit"></i></a> <a href="delProject.php?id='.$value[0].'" ';
    echo "onclick='";
